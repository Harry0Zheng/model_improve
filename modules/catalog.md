...
先读本表，再只打开 [library.md](library.md) 里对应的那一节。不要整份零件库读进上下文。

实现以 Markdown 为准：介绍 + 代码块都在 `library.md`。改实验代码时，按该节把模块写成项目里的独立 `nn.Module`，不要依赖本仓库的 `.py`。

来源说明：初始 26 条来自 B 站「DT算法工程师前钰」合集，只作零件，不构成研究贡献。后续自增模块追加在 `library.md` 文末，并在本表加一行。
...
筛选时默认只从「可用」里挑；「需核对」必须先对照 `library.md` 该节做 Forward，并写清接口再进入实验。
