...
结合 [modules/catalog.md](modules/catalog.md) 与本地 `e:\Codex模型改进\模块.md` 对应节。对每个候选写：核心作用、类型、插入位置、是否适合当前任务、Params/FLOPs、是否与现有模块重复、训练风险。只推荐 1～3 个首先实验的模块。只读选中的节，不要通读整份零件库。
